...
export class GirModule {
  ...
  constructor(xml: { repository: GirRepository }) {
  ...
  loadTypes(dict: InheritanceTable<any>) {
    ...
    if (this.ns.callback) for (let f of this.ns.callback) annotateFunctionArguments(f)
    let objs = (this.ns.class ? this.ns.class : [])
  ...
  loadInheritance<T>(inheritanceTable: InheritanceTable<T>) {
    // Class hierarchy
  ...
  private getReturnType(e) {
    ...
    let outArrayLengthIndex = returnVal.array && returnVal.array[0].$.length ? Number(returnVal.array[0].$.length) : -1
  ...
  private getSignalFunc(e: GirFunction, clsName: string) {
    ...
    return [
      `    connect(sigName: "${sigName}", callback: ((obj: ${clsName}${paramComma}${params}) => ${retType})): void`
    ]
  }
  ...
  private exportObjectInternal(e: GirClass | GirClass) {
    ...
    const copySignals = (cls: GirClass) => {
      ...
      if (signals) {
        ...
        for (let s of signals) def = def.concat(this.getSignalFunc(s, name))
      }
    }
    ...
    if (isDerivedFromGObject) {
      ...
      for (let p of propertyNames) {
        def.push(
          `    connect(sigName: "notify::${p}", callback: ((obj: ${name}, pspec: ${prefix}ParamSpec) => void)): void`
        )
      }
  ...
  export(outStream: NodeJS.WritableStream) {
    ...
    if (this.ns.enumeration) for (let e of this.ns.enumeration) out = out.concat(this.exportEnumeration(e))
    if (this.ns.bitfield) for (let e of this.ns.bitfield) out = out.concat(this.exportEnumeration(e))
    if (this.ns.constant) for (let e of this.ns.constant) out = out.concat(this.exportConstant(e))
    if (this.ns.function) for (let e of this.ns.function) out = out.concat(this.exportFunction(e))
    if (this.ns.callback) for (let e of this.ns.callback) out = out.concat(this.exportCallback(e))
    if (this.ns.interface) for (let e of this.ns.interface) out = out.concat(this.exportInterface(e))
    if (this.ns.class) for (let e of this.ns.class) out = out.concat(this.exportInterface(e))
    if (this.ns.record) for (let e of this.ns.record) out = out.concat(this.exportInterface(e))
    if (this.ns.union) for (let e of this.ns.union) out = out.concat(this.exportInterface(e))
    if (this.ns.alias) for (let e of this.ns.alias) out = out.concat(this.exportAlias(e))
